...
func TestBuildConfigChangeDetails_FlagsAndKeys(t *testing.T) {
	oldCfg := &config.Config{
		...
		MaxRetryInterval:       1,
		WebsocketAuth:          false,
		QuotaExceeded:          config.QuotaExceeded{SwitchProject: false, SwitchPreviewModel: false, DisableCredential: false, ReEnableAfter: ""},
	...
	newCfg := &config.Config{
		...
		MaxRetryInterval:       3,
		WebsocketAuth:          true,
		QuotaExceeded:          config.QuotaExceeded{SwitchProject: true, SwitchPreviewModel: true, DisableCredential: true, ReEnableAfter: "24h"},
		ClaudeKey: []config.ClaudeKey{
	...
	expectContains(t, details, "quota-exceeded.disable-credential: false -> true")
	expectContains(t, details, "quota-exceeded.re-enable-after: \"\" -> \"24h\"")
...
func TestBuildConfigChangeDetails_AllBranches(t *testing.T) {
	...
	expectContains(t, changes, "quota-exceeded.disable-credential: false -> true")
	expectContains(t, changes, "quota-exceeded.re-enable-after: \"\" -> \"24h\"")
